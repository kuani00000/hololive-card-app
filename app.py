...
st.markdown(
    """
    <style>
    /* スマホでドロップダウン(selectbox)タップ時にキーボードを起動させない設定 */
...
def load_data():
    if not os.path.exists("hololive_cards.xlsx"):
        st.error(
            "⚠️ `hololive_cards.xlsx` が見つかりません。`app.py` と同じフォルダに配置してください。"
        )
        return None
    try:
        ...
        st.error(
            "⚠️ `hololive_cards.xlsx` がExcel等で開かれています。ファイルを閉じてから再読み込みしてください。"
        )
        return None
...
def create_combined_offer_image(offered_list, cards_per_row=4):
    ...
    for idx, item in enumerate(offered_list):
        ...
        if card_img is None:
            card_img = Image.new(
                "RGB", (cell_size, cell_size), color=(235, 238, 242)
            )
...
def display_card_list(card_df, show_trade_count=False):
    ...
    for series_name, group_df in series_groups:
        ...
        collection_rate = (
            (owned_types / total_types * 100) if total_types > 0 else 0.0
        )

        st.markdown(
            f"#### 📦 {series_name} <span style='font-size:14px; color:#555; font-weight:normal;'>（収集率: {collection_rate:.1f}%）</span>",
            unsafe_allow_html=True,
        )
        ...
        for idx, row in group_df.iterrows():
            ...
            if pd.notna(img_filename) and str(img_filename).strip() != "":
                ...
                series_folder_path = os.path.join(
                    "images", str(series_name).strip(), filename
                )
...
def display_trade_card_grid(card_df, is_tradeable=True, cols_per_row=6):
    ...
    for series_name, group_df in series_groups:
        ...
        rows_data = [
            group_df.iloc[i : i + cols_per_row]
            for i in range(0, len(group_df), cols_per_row)
        ]
        ...
        for row_df in rows_data:
            ...
            for idx, (_, row) in enumerate(row_df.iterrows()):
                with cols[idx]:
                    ...
                    image_path = None
                    if (
                        pd.notna(img_filename)
                        and str(img_filename).strip() != ""
                    ):
                        filename = str(img_filename).strip()
                        series_folder_path = os.path.join(
                            "images", str(series_name).strip(), filename
                    ...
                    # メンバー名
                    st.markdown(
                        f'<div style="font-weight:bold; font-size:11px; line-height:1.1; margin:0 auto 2px auto; text-align:center; width:75px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;" title="{full_name}">{disp_name}</div>',
                        unsafe_allow_html=True,
                    )
                    ...
                    if img_b64:
                        st.markdown(
                            f'<img src="{img_b64}" style="width:75px; height:75px; object-fit:cover; border-radius:4px; display:block; margin:0 auto;" />',
                            unsafe_allow_html=True,
                        )
                    else:
                        st.markdown(
                            '<div style="width:75px; height:75px; background:#f0f0f0; border-radius:4px; display:flex; align-items:center; justify-content:center; font-size:10px; color:#888; margin:0 auto;">No Img</div>',
                            unsafe_allow_html=True,
                        )
                    ...
                    if is_tradeable:
                        st.markdown(
                            f'<div style="font-size:11px; font-weight:bold; color:#e67e22; margin-top:2px; text-align:center; width:75px; margin-left:auto; margin-right:auto;">{trade_qty}枚可</div>',
                            unsafe_allow_html=True,
                        )
                        unique_btn_key = (
                            f"add_trade_{series_name}_{full_name}_{row.name}"
                        )
                        if st.button(
                            "➕提供",
                            key=unique_btn_key,
                        ...
                        st.markdown(
                            '<div style="font-size:10px; color:#666; margin-top:2px; text-align:center; width:75px; margin-left:auto; margin-right:auto;">未所持</div>',
                            unsafe_allow_html=True,
                        )
        st.markdown("---")
...
if menu == "🏠\nホーム":
    ...
    st.info(
        "👇 画面下のメニューから「📊 収集率」や「🎴 所持リスト」を選択してください。"
    )
    ...
    if df is not None:
        ...
        with col2:
            st.metric(
                "総所持枚数", f"{total_cards} 枚", f"内 開封済 {total_opened} 枚"
            )
...
# 2. 収集率一覧画面
    ...
    if df is None:
        st.warning(
            "Excelデータが読み込まれていないため、収集率を表示できません。"
        )
    else:
        total_all_types = len(df)
        total_owned_types = (df["所持数"] > 0).sum()
        total_rate = (
            (total_owned_types / total_all_types * 100)
            if total_all_types > 0
            else 0.0
        )

        st.markdown(
            f"### 🌐 全体コンプリート率: **{total_rate:.1f}%** ({total_owned_types}/{total_all_types}種類)"
        )
        ...
        for series_name, group_df in series_groups:
            ...
            with col_img:
                if img_b64:
                    st.markdown(
                        f'<img src="{img_b64}" style="width:75px; height:75px; object-fit:cover; border-radius:6px; display:block; margin-top:4px;" />',
                        unsafe_allow_html=True,
                    )
                else:
                    st.markdown(
                        '<div style="width:75px; height:75px; background:#f0f0f0; border-radius:6px; display:flex; align-items:center; justify-content:center; font-size:10px; color:#888; margin-top:4px;">No Img</div>',
                        unsafe_allow_html=True,
                    )
            ...
            with col_info:
                st.markdown(f"#### 📦 {series_name}")
                st.markdown(
                    f"**収集率: {rate:.1f}%** （{owned_types} / {total_types} 種類）"
                )
                st.caption(
                    f"所持枚数: {s_owned_cards}枚 (開封:{s_opened_cards}枚) | 未所持: {unowned_types}種類"
                )
...
# 3. 所持リスト（カード検索画面）
    ...
    if df is None:
        st.warning(
            "Excelデータが読み込まれていないため、カード検索機能を利用できません。"
        )
    else:
        raw_members = df["メンバー名"].dropna().unique().tolist()
        base_members = sorted(
            list(
                set([re.sub(r"[\d①-⑨]+$", "", str(m)).strip() for m in raw_members])
            )
        )
        ...
        if search_type == "キャラ別検索":
            selected_name = st.selectbox(
                "メンバーを選択してください", member_options
            )
            if selected_name != "-- メンバーを選択してください --":
                filtered_df = df[
                    df["メンバー名"]
                    .astype(str)
                    .str.contains(selected_name, na=False)
                ].copy()
                title_text = f"「{selected_name}」のカード一覧"

        else:
            selected_series = st.selectbox(
                "シリーズを選択してください", series_options
            )
        ...
        if not filtered_df.empty:
            ...
            st.caption(
                f"全 {len(filtered_df)} 種類 | 所持: {len(owned_df)} 種類 ({total_owned_qty}枚 / 内 開封済: {total_opened_qty}枚) | 未所持: {len(unowned_df)} 種類 | トレード可: {len(tradeable_df)} 種類"
            )
...
# 4. トレード画面
    ...
    if df is None:
        st.warning(
            "Excelデータが読み込まれていないため、トレード機能を利用できません。"
        )
    else:
        ...
        with col_search:
            ...
            if not filtered_df.empty:
                ...
                with tab1:
                    st.info(
                        "💡 「➕提供」ボタンをクリックすると右側の「提供リスト」に追加されます。"
                    )
                    display_trade_card_grid(
                        tradeable_df, is_tradeable=True, cols_per_row=6
                    )

                with tab2:
                    st.info("💡 トレードで探している未所持カードの一覧です。")
                    display_trade_card_grid(
                        unowned_df, is_tradeable=False, cols_per_row=6
                    )
        ...
        with col_offer:
            ...
            if offered_list:
                ...
                with btn_col1:
                    if st.button(
                        ...
                        st.session_state.generated_image_bytes = (
                            create_combined_offer_image(offered_list)
                        )
                ...
                st.markdown(
                    '<div class="offer-container">', unsafe_allow_html=True
                )
                ...
                for idx, item in enumerate(offered_list):
                    ...
                    with c_img:
                        img_b64 = get_cropped_square_base64(
                            item.get("image_path")
                        )
                        if img_b64:
                            st.markdown(
                                f'<img src="{img_b64}" style="width:75px; height:75px; object-fit:cover; border-radius:4px; display:block;" />',
                                unsafe_allow_html=True,
                            )
                        else:
                            st.markdown(
                                '<div style="width:75px; height:75px; background:#f0f0f0; border-radius:4px; display:flex; align-items:center; justify-content:center; font-size:10px; color:#888;">No Img</div>',
                                unsafe_allow_html=True,
                            )
                ...
                st.info(
                    "👈 左側一覧で「➕提供」を押すと、ここに提供用カードが追加されます。"
                )